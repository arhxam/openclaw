...
Archiving a non-main cloud-worker session also performs this safe stop and reclaim before the Gateway records it as archived. Restoring the session retains the reclaimed placement metadata so the next turn can dispatch a fresh worker with the same workspace profile.
