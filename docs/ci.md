...
| `blacksmith-8vcpu-ubuntu-2404`  | Retained heavy Linux Node suites, the ordinary packed Chromium/Vite `checks-ui-e2e` shards, boundary/extension-heavy `check-additional-*` shards, and `android`                                                                                                                   |
| `blacksmith-16vcpu-ubuntu-2404` | Automatic QA Smoke CI shards, the dedicated `checks-ui-e2e-real-gateway` lane, `build-artifacts` in CI and Testbox, and `check-lint` (CPU-sensitive enough that 8 vCPU cost more than they saved)                                                                                 |
